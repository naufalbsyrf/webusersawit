...
function _0x1528(_0x4a9756, _0x4fa616) {
  ...
  const querys = db.collection("Event");
  querys
    .get()
        ...
        db.collection("Event")
          ...
          .then((commentQuerySnapshot) => {
            ...
            if (Event.length === articleQuerySnapshot.size) {
              ...
              const topEvent = Event.slice(1, 7);
      ...
      querySnapshot.forEach((doc) => {
        ...
        kanbanList.innerHTML = `
                <div class="kanban-boxartikel item-box">
                    <div class="">
                        <a href="./detailEvent.html?id=${laporanData.id}">
                            <img src="${laporanData.img || ""}" id="imgLaporanpopuler" data-articleid="${laporanData.id}" />
                        </a>
                    </div>
                    <div class="" id="laporanContainer">
                        <h5>
                          <a href="./detailEvent.html?id=${laporanData.id}" id="titlelaporanpopuler">
                            ${truncateString(laporanData.title, 70) || ""}
                          </a>
                        </h5>
                        <span id="titlelaporanpopulerkonten">${truncateString(laporanData.content,200) || ""}</span>
                        <span id="titlelaporanpopulerkonten1">${"Tanggal Acara: "+formattedDate || ""}</span>
                    </div>
                </div>
              </div>
                `;
  ...
  function truncateString(str, maxLength) {
    if (window.innerWidth <= 767) {
      // Jika lebar jendela kurang dari atau sama dengan 767, atur maxLength ke 40
      maxLength = 40;
    }
    if (window.innerWidth >= 767) {
      maxLength = 150;
        ...
        db.collection("Event")
          ...
          .then((commentQuerySnapshot) => {
            ...
            if (Event.length === articleQuerySnapshot.size) {
              ...
              // Ambil 6 artikel dengan komentar paling banyak
              const topEvent = Event.slice(0, 1);
              ...
              <p class="text-detail-threads">
                  <span style="font-weight: bold;">${"Tanggal Acara: "+formattedDate}</span> 
                  </p>
                </div>
                <div class="me-auto">
                  <p class="text-detail-threads">
