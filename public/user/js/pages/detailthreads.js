...
db.collection("threads")
  ...
  .then((doc) => {
    if (doc.exists) {
      ...
      let title = document.getElementById("titleThreads");
      let kronologisingkat = document.getElementById("kronologisingkat");
      let imgElement = document.getElementById("imgThreads"); // Menyesuaikan ID elemen gambar Anda

      title.textContent = threadData.title;
      idUserThread = threadData.userId;
      kronologisingkat.textContent = threadData.kronologisingkat;
...
var konsultasiButton = document.getElementById("startKonsultasi");
console.log(konsultasiButton);
konsultasiButton.addEventListener("click", function () {
  // Menggunakan window.location.href untuk navigasi
  window.location.href = `konsultasi.html?id=${idUserThread}`;
});
//sidebar
document.addEventListener("DOMContentLoaded", function () {
