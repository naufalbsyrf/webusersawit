...
function _0x1528(_0x4a9756, _0x4fa616) {
  ...
  const querys = db.collection("threads");
  querys
    .get()
      ...
      articleQuerySnapshot.forEach((articleDoc) => {
        ...
        db.collection("threads")
          ...
          .then((commentQuerySnapshot) => {
            ...
            threads.push({ article: articleData, commentCount });
  
            // Jika sudah mendapatkan jumlah komentar untuk semua artikel
            if (threads.length === articleQuerySnapshot.size) {
              ...
              const topthreads = threads.slice(1, 7);
  
              // Tampilkan artikel paling banyak komentarnya
              topthreads.forEach((article) => {
    ...
    .then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
        ...
        kanbanList.innerHTML = `
                ...
                <img src="${
                  laporanData.img || ""
                }" id="imgLaporanpopuler" data-articleid="${laporanData.threadsId}" />
              </a>
                </div>
                <div class="">
                  <h5>
                    <a href="./detailOpini.html?id=${laporanData.threadsId}" id="titlelaporanpopuler">
                      ${truncateString(laporanData.title, 70) || ""}
                    </a>
                  </h5>
                  <span id="titlelaporanpopulerkonten">${
                    truncateString(laporanData.kronologisingkat, 40) || ""
                  }</span>
        ...
        db.collection("threads")
          ...
          .then((commentQuerySnapshot) => {
            ...
            if (threads.length === articleQuerySnapshot.size) {
              ...
              // Ambil 6 artikel dengan komentar paling banyak
              const topthreads = threads.slice(0, 1);
              ...
              topthreads.forEach((article) => {
                ...
                <img src="${
                  laporanData.img || ""
                }" id="imgLaporanpopuler1" data-articleid="${laporanData.threadsId}"/>
              </a>
              <p class="text-detail-threads">
                ...
                <div class="me-auto">
                  <p class="text-detail-threads">
                    <span id="description">${
                      truncateString(laporanData.kronologisingkat, 200) || ""
                    } </span>
                  </p>
                </div>
