...
var firebaseConfig = {
  ...
  db.collection("Event")
    ...
    .then((doc) => {
      if (doc.exists) {
        ...
        let tanggalAcara = document.getElementById("tanggalAcara");
        let linkAcara = document.getElementById("linkAcara");
        let namaAcara = document.getElementById("namaAcara");
        ...
        namaAcara.textContent = "Nama Acara: "+threadData.title;
        tanggalAcara.textContent = "Tanggal Acara: " +threadData.tanggal;
        linkAcara.addEventListener('click', function() {
            window.location.href = threadData.link;
          });
          
          // Opsional: Mengubah cursor menjadi pointer saat mouse berada di atas span
        linkAcara.style.cursor = 'pointer';
    ...
    .then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
        ...
        <div class="kanban-box item-box">
          <div class="img-news">
            <a href="detailEvent.html?id=${doc.id}">
              <img src="${laporanData.img || ""}" id="imgLaporan" />
            </a>
          </div>
          <div class="kanban-header">
            <h5>
              <a href="detailEvent.html?id=${laporanData.id}">
  ...
  const query = db.collection("Event");
  ...
  topEvent.forEach((article) => {
    ...
    document.title = truncateString(laporanData.title, 100) || "";
