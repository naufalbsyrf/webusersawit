...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var addThreadsModal = document.getElementById("add_laporan"); // Modal "Upload Threads"
        ...
        querySnapshot.forEach((doc) => {
          ...
          <div class="kanban-header-threads">
                ...
                <div class="dropdown-menu dropdown-menu-end">
                  ...
                  <a class="dropdown-item" data-target="#add_project11${
                    laporanData.reportId
            ...
            laporanData.description
              ...
              <h5><span id="titlelaporan">${laporanData.title || ""}</span></h5>
            </div>
            <div class="kanban-wrap">
              <div>
                <div class="kanban-box item-box">
...
function updateFirestoreThread(threadsId) {
  ...
  const titleInput = document.getElementById(`titlelaporan${threadsId}`).value;
  const lokasikejadian = document.getElementById(
    `lokasikejadian${threadsId}`
  ).value;
  ...
  const imgInput = document.getElementById(`imgLaporan${threadsId}`);
  ...
  const threadRef = db.collection("reports").doc(threadsId);
  threadRef
    .get()
    .then((doc) => {
      if (doc.exists) {
        ...
        const updateData = {
          address: lokasikejadian,
          title: titleInput,
          description: kronologisingkatTextarea,
          date: {
...
function postToFirestore() {
  ...
  var description = document.getElementById("deskripsi").value;
  var imageFile = document.getElementById("imglaporan").files[0];
  var address = document.getElementById("lokasikejadian").value;

  if (imageFile) {
    let uploadTask = storage.child("report/" + imageFile.name).put(imageFile);
      ...
      function () {
        uploadTask.snapshot.ref.getDownloadURL().then(function (downloadURL) {
          ...
          if (user) {
            ...
            // Dapatkan nomor telepon pengguna dari koleksi "users"
            db.collection("users")
              .doc(userId)
              .get()
              .then(function (userDoc) {
                if (userDoc.exists) {
                  ...
                  var laporanData = {
                    ...
                    address: address,
                    userId: userId,
                    phoneNumber: userPhoneNumber, // Tambahkan nomor telepon ke dalam objek laporanData
                    date: {
                      createdDate:
